...
MODULE_LICENSE("GPL");
MODULE_AUTHOR("Michael Rozentsveig");
...
static inline void mq_ctor(struct messageQu * mq)
{
	mq->size = 0;
	mq->capacity = 100;
	INIT_LIST_HEAD(&(mq->head));
	mq->mq_device=NULL;
    init_waitqueue_head(&mq->read_queue);
    mutex_init(&mq->lock);
}

static inline void mq_dtor(const struct messageQu * mq)
{
    struct node* current_message;
    list_for_each_entry(current_message,&mq->head,node)
    {
