...
void destroyAD(struct Calendar_t* ca){ /*Frees the memory of the struct and the array*/
...
void loadFromFile(struct Calendar_t* cal){
	float begin,end;
	int room;	
	...
	if (fp != NULL){
		while (1){
			...
			if (!feof(fp)){
				me=createMeetingToFile(begin,end,room);	
				insertAppo(cal,me);	
			}
		}
		fclose(fp);
...
void saveInFile(struct Calendar_t* cal){
	int i = 0;
	FILE* fp=fopen("Meetings.txt","w");
